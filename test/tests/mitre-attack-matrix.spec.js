...
test.describe('MITRE ATT&CK Matrix Page', () => {
  ...
  test('should display the page title and header correctly', async ({ page }) => {
    ...
    // Check main heading
    await expect(page.getByRole('heading', { name: 'MITRE ATT&CK Matrix for E-Skimming' })).toBeVisible();
    
    // Check version badge
    await expect(page.getByText('v1.0')).toBeVisible();
    
    // Check subtitle
    await expect(page.getByText('Comprehensive framework for understanding, detecting, and defending against e-skimming attacks')).toBeVisible();
  });
  ...
  test('should display "No techniques" message for empty tactics', async ({ page }) => {
    ...
    await expect(noTechniquesMessages.first()).toContainText('No techniques commonly used in e-skimming attacks');
  });
  ...
  test('should have proper footer content', async ({ page }) => {
    ...
    // Check footer text
    await expect(footer.getByText('MITRE ATT&CK Matrix for E-Skimming Attacks | Version 1.0 | Last Updated: 2025-01-18')).toBeVisible();
    await expect(footer.getByText('Based on extensive research and analysis of real-world e-skimming campaigns')).toBeVisible();
  });
