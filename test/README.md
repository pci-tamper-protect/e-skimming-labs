...
This directory contains Playwright tests for the E-Skimming Labs platform,
focusing on testing the MITRE ATT&CK Matrix page and other core functionality.
...
- **Environment Detection**: Tests that the JavaScript correctly detects the
  environment (localhost, staging, production) and sets the appropriate URL
- **URL Verification**: Confirms the back button href is set correctly for each
  environment
- **Navigation**: Tests that clicking the back button navigates to the correct
  page
- **Console Logging**: Verifies that environment detection is logged to console

### Matrix Table Tests

- **Column Count**: Verifies exactly 12 columns (one for each MITRE ATT&CK
  tactic)
...
- **Technique Display**: Tests that techniques and sub-techniques are properly
  formatted
...
2. **Timeout Errors**: Check if the service is responding at
   `http://localhost:3000`
3. **Element Not Found**: Verify the page has loaded completely with
...
- **Technique Names**: "Exploit Public-Facing Application", "Valid Accounts",
  etc.
