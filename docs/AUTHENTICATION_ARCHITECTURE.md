...
2. ForwardAuth middlewares (`lab1-auth-check`, etc.) are written at runtime by the entrypoint when `HOME_INDEX_URL` is set:
   - **Sidecar deploy** (traefik + provider containers): `deploy/traefik/deprecated-v2/entrypoint-sidecar.sh` writes `/shared/traefik/dynamic/auth-forward.yml`. Set `HOME_INDEX_URL` in `cloudrun-sidecar.yaml`; `deploy-sidecar.sh` and `deploy-sidecar-traefik-3.0.sh` substitute the home-index service URL when deploying.
