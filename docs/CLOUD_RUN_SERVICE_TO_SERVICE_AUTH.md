...
3. **Service A** makes HTTP request to **Service B** with token:
   ...
   - Header: **`X-Serverless-Authorization: Bearer <identity-token>`** (used by this project's Traefik provider)
   - Note: Cloud Run also accepts `Authorization: Bearer <identity-token>`, but this project uses `X-Serverless-Authorization` to avoid conflicting with the user's Firebase JWT in the `Authorization` header.
...
# Note: use X-Serverless-Authorization (not Authorization) to preserve the Authorization header for user Firebase JWTs
TOKEN=$(gcloud auth print-identity-token --audiences="https://service-207478017187.us-central1.run.app")
curl -H "X-Serverless-Authorization: Bearer $TOKEN" https://service-207478017187.us-central1.run.app/
...
The Traefik Cloud Run provider generates identity tokens and injects them as `X-Serverless-Authorization: Bearer <token>` (not `Authorization: Bearer`). This preserves the `Authorization` header for user Firebase JWTs, which home-index reads for its own authentication checks. Cloud Run accepts identity tokens in both headers; using `X-Serverless-Authorization` eliminates header conflicts.
