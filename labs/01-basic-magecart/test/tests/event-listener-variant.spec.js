...
test.describe('Lab 1: Basic Magecart - Event Listener Variant', () => {
  ...
  test('should collect data via event listeners instead of form submission', async ({ page }) => {
    ...
    const variantHtmlPath = path.join(
      __dirname,
      '../../variants/event-listener-variant/vulnerable-site/checkout.html'
    )
    ...
    const variantJsPath = path.join(
      __dirname,
      '../../variants/event-listener-variant/vulnerable-site/js/checkout-compromised.js'
    )
    ...
    console.log(
      '✅ Form submitted successfully - event listeners captured data during field interactions'
    )
    ...
    console.log(
      '🔍 Test completed - data should have been collected via mouseup/touchend/blur events'
    )
  })
