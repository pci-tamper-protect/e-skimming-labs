...
test.describe('E-Skimming Lab - WebSocket Exfiltration Variant', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('request', request => {
      if (request.url().includes('localhost:3000/collect') || request.url().includes('localhost:3001/ws')) {
    ...
    page.on('response', response => {
      if (response.url().includes('localhost:3000/collect') || response.url().includes('localhost:3001/ws')) {
