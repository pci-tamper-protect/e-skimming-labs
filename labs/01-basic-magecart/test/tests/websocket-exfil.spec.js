...
test.describe('E-Skimming Lab - WebSocket Exfiltration Variant', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    page.on('request', request => {
      if (request.url().includes('localhost:9002/collect') || request.url().includes('localhost:3001/ws')) {
    ...
    page.on('response', response => {
      if (response.url().includes('localhost:9002/collect') || response.url().includes('localhost:3001/ws')) {
  ...
  test('should attempt WebSocket connection and fallback to HTTP', async ({ page }) => {
    ...
    const variantHtmlPath = path.join(__dirname, '../../variants/websocket-exfil/vulnerable-site/checkout.html');
    let htmlContent = fs.readFileSync(variantHtmlPath, 'utf8');

    // Update the script src to point to the variant's JS file
    const variantJsPath = path.join(__dirname, '../../variants/websocket-exfil/vulnerable-site/js/checkout-compromised.js');
    ...
    console.log('✅ Form submitted successfully - WebSocket variant should attempt connection and fallback to HTTP');

    // Wait for skimmer to process
    await page.waitForTimeout(3000);

    console.log('🔍 Test completed - WebSocket connection should have been attempted with HTTP fallback');
  });
  ...
  test('should contain WebSocket communication patterns', async ({ page }) => {
    ...
    const fs = require('fs');
    const path = require('path');
    const jsFilePath = path.join(__dirname, '../../variants/websocket-exfil/vulnerable-site/js/checkout-compromised.js');
    ...
    const hasReconnectionLogic = jsContent.includes('reconnectAttempts') && jsContent.includes('exponential');
    console.log('✓ Reconnection logic found:', hasReconnectionLogic);
    expect(hasReconnectionLogic).toBe(true);

    // Check for HTTP fallback
    const hasHTTPFallback = jsContent.includes('fallbackToHTTP') || jsContent.includes('fallbackUrl');
    ...
    const hasStructuredMessaging = jsContent.includes('createWebSocketMessage') && jsContent.includes('JSON.stringify');
  ...
  test('should capture data with WebSocket communication attempt', async ({ page }) => {
    ...
    const wsMessages = consoleMessages.filter(msg =>
      msg.includes('WebSocket') || msg.includes('ws://') || msg.includes('fallback')
    );
    ...
    const initMessages = consoleMessages.filter(msg =>
      msg.includes('WebSocket skimmer') || msg.includes('Establishing WebSocket')
    );
  ...
  test('should handle WebSocket connection lifecycle', async ({ page }) => {
    ...
    const connectionAttempts = consoleMessages.filter(msg =>
      msg.includes('Establishing WebSocket') || msg.includes('WebSocket connection')
    );
    ...
    const errorMessages = consoleMessages.filter(msg =>
      msg.includes('WebSocket error') || msg.includes('connection closed') || msg.includes('Failed to create')
    );
    ...
    const fallbackMessages = consoleMessages.filter(msg =>
      msg.includes('fallback') || msg.includes('HTTP POST')
    );
    ...
    const reconnectMessages = consoleMessages.filter(msg =>
      msg.includes('reconnection') || msg.includes('Attempting reconnection')
    );
