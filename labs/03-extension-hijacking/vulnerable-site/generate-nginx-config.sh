...
server {
    ...
    # Production: serve lab versions with warnings (index.html)
    # Staging: serve training versions without warnings (index-train.html)
    ...
    location = / {
        # Rewrite root to training version if staging
        if (\$host ~* "stg\.pcioasis\.com") {
