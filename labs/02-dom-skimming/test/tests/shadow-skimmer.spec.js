...
test.describe('Lab 2: DOM-Based Skimming - Shadow DOM Stealth Attack', () => {
  ...
  test('should monitor fields across shadow boundaries', async ({ page }) => {
    ...
    if (capturedData && capturedData.length > 0) {
      const sessionWithData = capturedData.find(
        session => session.values.length > 0 || session.keystrokes.length > 0
      )
    ...
    const shadowRequests = networkRequests.filter(req => {
      if (req.postData) {
        try {
          ...
          return (
            data.type === 'shadow_dom_capture' || data.metadata?.attackType === 'shadow-dom-stealth'
          )
        } catch (e) {
          return false
        }
  ...
  test('should handle nested shadow DOM structures', async ({ page }) => {
    ...
    const shadowStructure = await page.evaluate(() => {
      const shadowInfo = window.shadowSkimmerAttack
        ? window.shadowSkimmerAttack.getShadowHosts()
        : []
