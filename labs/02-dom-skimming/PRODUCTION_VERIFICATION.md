...
- **Navigation buttons moved above tabs**: `nav-buttons-row` is now separate
  from `lab-tabs`
- **Settings tab removed**: Only 4 tabs remain (Dashboard, Transfer, Bill Pay,
  Cards)
- **Cards page is default**: `cards` section is active by default
- **Add card form visible by default**: `add-card-form-container` is visible on
  cards page
...
The production Dockerfile (`labs/02-dom-skimming/Dockerfile`) copies the entire
`vulnerable-site/` directory:
...
4. `labs/02-dom-skimming/vulnerable-site/Dockerfile` - Local dev Dockerfile
   (uses public base image)

**Note**: Production uses `labs/02-dom-skimming/Dockerfile` which already copies
`vulnerable-site/`, so all changes will be included automatically.
